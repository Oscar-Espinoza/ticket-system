...
## Round 2 additions (see `10-MASTER-PLAN-2.md` and the `D*-*.md` plan docs)
- **Issue model/service**: `IssueRow.startDate`, `slaDueAt`, `slaBreachedAt`, `stateChangedAt`; `IssuePatch.startDate`; `slaDeadline()`, `moveIssue()`, `epicAvailableTo()`, `availableEpicsForProject()` in `src/lib/issue-service.ts`; `createIssue(..., { source: 'import' })` marks events `data.bulk`. `getIssueByKey(projectId, number, viewerId?)` follows `ticket_key_alias`.
- **Editor**: `MarkdownEditor` is Tiptap now (same props + `extensions`, `history`, `controlled`, `onEditor`, `ticketId`, `issues`); `RichEditorProvider` sets upload/issue context; `Markdown` has `embeds`. Uploads: `POST /api/uploads`, `GET /api/uploads/[uploadId]`.
- **Collaboration**: `src/lib/collab/provider.ts` (`getCollabProvider`, `retainCollabProvider`), `useCollab(key, user)`; keys `doc:<id>` / `issue:<id>`; `GET|POST /api/collab/[key]`.
- **Sync**: `src/lib/sync/outbox.ts` (`perform`, `flush`, `useOutboxItems`), `getChangeToken()`, SSE `GET /api/projects/[id]/stream`.
- **Notifications**: `deliverToChannels(rows)` (`src/lib/notifications/channels`) after inserting notification rows directly; types `sla_breached`, `recurring_created`, `pulse`; `prefEnabled(prefs, type)`.
- **Cron**: `/api/cron/daily` → `src/lib/cron/jobs/*` (automations, recurring, sla, digests, pulse, webhookRetries, collabPrune).
- **Search/filters**: query syntax parsed by `src/lib/issue-filtering.ts` (`issueMatcher`); `searchDocuments()` in `src/lib/documents.ts`.
- **Other shared helpers**: `findSimilarIssues()` (`src/lib/similarity.ts`), `customerRequestsForIssues()` / `matchCustomerByEmail()` (`src/lib/customers.ts`), `recordWorkspaceEvent()` / `recordProjectEvent`-style audit helpers (`src/lib/audit.ts`), `authenticateApiRequest()` (API keys + OAuth tokens, scopes), `resolveIssueKey()`, `listProjectIssues()`, webhook delivery helpers in `outgoing-webhooks.ts`, `src/lib/vcs/automation.ts` (shared GitHub/GitLab/Bitbucket PR automation).
- Favorite targets now include `dashboard`.
